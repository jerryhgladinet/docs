...
|prodname| server is built on top of the Microsoft Web Platform, which includes:
...
It is easier and faster to install |prodname| software on Windows Server 2012/2012 R2 or on Windows Server 2016/2019.

.. note::

    The |prodname| server installer is capable of installing all the dependency system components such as .NET Framework and etc. For installation, a clean machine is recommended.
...
It will be loaded with the mentioned Microsoft components before the core |prodname| 
software is installed. 

The |prodname| installer will install and load  all the dependency Windows components.
...
If you are setting |prodname| server up in Amazon AWS, Amazon Aurora DB is also recommended.

.. note::

    The |prodname| server installer is capable of installing SQL Express or MySQL. If you only need an all-in-one deployment for a single server deployment, the installer can install the database automatically.

.Net Framework 4
^^^^^^^^^^^^^^^^^

The |prodname| Server is built with .Net Framework 4, with ASP.NET and WCF.

We recommend .Net Framework 4.5 and above as it works better with remote clients that have Internet Explorer 10 or 11.

.. note::

    The |prodname| server Installer will install .NET 4.5 and other
    dependency components automatically. 

ASP.NET 4.5/4.6/4.7
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

|prodname| web browser portal user interface is written in ASP.NET, HTML and JavaScript.

Internet Information Server (IIS)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

|prodname| Server-side services are hosted inside the Internet Information Server (IIS).
...
.. note::
    ...
    The |prodname| Server Installer will enable IIS Service if not already. So no manual step required.
...
.. note:: 
    ...
    Virtual Machines are recommended. A clean machine built from a base OS with English Locale is recommended. After providing the clean machine, the |prodname| Server installer will install the rest of the Windows 
    system components and |prodname| components automatically. 
...
For user counts smaller than 1000, a single |prodname| server is enough. Most of the time, an all-in-one server deployment will be sufficient. In this case you will install the |prodname| Server in a single physical or virtual machine.
The default |prodname| installer will install all the components (Database/Web Node/Worker Node) on one machine.
...
However, |prodname| is capable of using worker nodes (for web function only) for load-balancing if you do not have an existing load balancer. You can pick a worker node to be the node exposed for login/contact
